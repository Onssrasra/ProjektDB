// scraper.js - A2V-only scraper with structured JSON extraction from window.initialData
...
function extractJsonInitialData(html) {
  // Try to extract window.initialData['product/dataProduct'] = {...};
...
function mapFromInitialData(obj, a2v, url) {
  try {
    ...
    // Helper to get TS by label includes
    ...
    // Gewicht: TS oder numeric product.weight (kg)
    let gewicht = pickTs('gewicht') || null;
    if (!gewicht && typeof product.weight === 'number') {
      gewicht = `${product.weight.toString().replace('.', ',')} kg`;
    }
    ...
    return {
      ...
      Gewicht: gewicht,
      Abmessung: 'Nicht gefunden',  // laut Beispiel nicht vorhanden
      Werkstoff: 'Nicht gefunden',  // laut Beispiel nicht vorhanden
...
class SiemensProductScraper {
  ...
  async pwScrapeA2V(a2v) {
    ...
    // Extract initial data via DOM if available
    ...
    // Fallback: Cheerio-like parsing via page
  ...
  async scrapeOne(a2v) {
    ...
    if (!key.startsWith('A2V')) {
      throw new Error('Nur A2V-Nummern sind erlaubt.');
    }
